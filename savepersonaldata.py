...
for i in range(2, formWs.max_row+1):
    if formWs.cell(i, 2).value is not None:
        ...
        teacher = str(formWs.cell(i, 4).value)
        testName = str(formWs.cell(i, 5).value)
        average = formWs.cell(i, 7).value
    # 시험 미응시시 건너뛰기
    if formWs.cell(i, 6).value is None: continue
    
    # 해당 학생 엑셀 파일이 존재하지 않으면 생성
    if not os.path.isfile(config['dailyTestPersonalFilePath'] + str(formWs.cell(i, 3).value) + '.xlsx'):
        iniWb = xl.Workbook()
        iniWs = iniWb.active
        iniWs['A1']='응시일'
        iniWs['B1']='반'
        ...
        iniWs['E1']='점수'
        iniWs['F1']='반평균'
        iniWb.save(config['dailyTestPersonalFilePath'] + str(formWs.cell(i, 3).value) + '.xlsx')

    # 해당 학생 파일에 응시 결과 입력
    studentWb = xl.load_workbook(config['dailyTestPersonalFilePath'] + str(formWs.cell(i, 3).value) + '.xlsx', data_only=True)
    ...
    studentWs.cell(writeLocation, 5).value = formWs.cell(i, 6).value
